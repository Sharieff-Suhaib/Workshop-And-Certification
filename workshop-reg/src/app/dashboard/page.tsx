...
// ── Mock data (replace with React Query calls later) ──────────────────────────
const MOCK_USER = { name: 'Kulasekaran', email: 'kula@annauniv.edu' };
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    ...
    >
      <span className="rounded-full" style={{ width: '5px', height: '5px', background: cfg.dot, flexShrink: 0 }} />
      {cfg.label}
    </span>
  );
}
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-2 gap-4" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
        <StatCard
          ...
          value={MOCK_WORKSHOPS.length}
          accent="#f472b6"
          sub="Across all workshops"
          icon={
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          {/* Filters (only for workshops tab) */}
          {activeTab === 'workshops' && (
            <div className="flex items-center gap-2">
              <span style={{ color: '#4b4b58', fontSize: '12px' }}>Status:</span>
              {['All', 'Registered', 'Attended', 'Certified'].map((s) => (
                <button key={s} style={FILTER_BTN(statusFilter === s)} onClick={() => setStatusFilter(s)}>
                  {s}
                </button>
              ))}
              ...
              {['All', 'Technical', 'Non-Technical'].map((c) => (
                <button key={c} style={FILTER_BTN(categoryFilter === c)} onClick={() => setCategoryFilter(c)}>
                  {c}
                </button>
              ))}
            </div>
          ...
          <div className="rounded-xl overflow-hidden" style={{ border: '1px solid #1e1e24' }}>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((w, i) => (
                ...
                >
                  <span className="font-medium text-sm" style={{ color: '#e0e0e8' }}>
                    {w.title}
                  </span>
                ...
                {certified.map((w) => (
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between">
                      <div
                        className="rounded-lg flex items-center justify-center"
                        style={{ width: '40px', height: '40px', background: 'rgba(52,211,153,0.08)', color: '#34d399' }}
                      >
                      ...
                      <span
                        ...
                        style={{ background: '#0c0c0f', color: '#4b4b58', border: '1px solid #1e1e24' }}
                      >
                        {w.certificateId}
                      </span>
                    </div>
                    ...
                    <div className="flex gap-2 pt-1" style={{ borderTop: '1px solid #1a1a20' }}>
                      <button
                        className="flex-1 flex items-center justify-center gap-2 text-xs font-medium rounded-lg py-2 transition-all"
                        style={{
                      ...
                      <button
                        className="flex-1 flex items-center justify-center gap-2 text-xs font-medium rounded-lg py-2"
                        style={{
                          background: '#18181f',
                          color: '#6b6b7a',
                          border: '1px solid #1e1e24',
